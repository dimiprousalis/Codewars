...
function testResult(array) {
    let avg = Number((array.reduce((a, c) => a + c, 0) / array.length).toFixed(3))
    let h = array.filter(v => v > 8).length
    let a = array.filter(v => v > 6 && v < 9).length
    let l = array.filter(v => v < 7).length
    ...
    return a == 0 && l == 0 ? [avg, dictionary, "They did well"] : [avg, dictionary]
}